...
<script>
  import {
    backstagePgAccountManagementDropListHttp,
    backstagePgAccountManagementHttp,
    backstagePgAccountManagementAdjustHttp,
    backstagePgAccountManagementInternalTransferHttp,
  ...
  export default {
        ...
        tableConfig: {
          ...
          tableColumn: [{
            ...
            render: (h, params) => {
              ...
              // })
              let routeData = _this.$router.resolve({
                name: 'bank_management_pg_account_management_detail_and_history',
                query: {
                  id: params.item.id,
    ...
    methods: {
      ...
      DropListHttp () {
        ...
        backstagePgAccountManagementDropListHttp().then(res => {
          this.DropList = res
          ...
          filter.formValueListFilter(this.AdjustformConfig, res, ['is_income', 'account_id'])
          // filter.formValueListFilter(this.BufferformConfig, res, ['is_income', 'account_id'])
          this.InternalformConfig[0].list = res.account_id
          this.InternalformConfig[2].list = res.bank
        }).catch(() => {
        })
      },
      AdjustChangeEvent (value) {
        if (value.value) {
          backstagePgAccountManagementDetailHttp(value.value).then(res => {
      ...
      changeEvent (value) {
        ...
        } else if (value.formValue === 'to_account_id'){
          backstageCompanyBankAccountsDetailHttp(value.value).then(res => {
            if (value.formValue === 'from_account_id') {
              this.InternalformConfig[1].value = res.balance
            } else {
      ...
      AdjustonSubmit (form) {
        delete form.balance
        backstagePgAccountManagementAdjustHttp(form).then(res => {
      ...
      InternalonSubmit (form) {
        ...
        backstagePgAccountManagementInternalTransferHttp(datalist).then(res => {
      ...
      initHttp (formInline) {
        ...
        backstagePgAccountManagementHttp(datalist).then(res => {
