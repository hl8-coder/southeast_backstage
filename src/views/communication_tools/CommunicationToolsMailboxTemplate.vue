...
import {
  ...
  backstageMailboxTemplateHttp,
  backstageMailboxTemplateStoreHttp,
...
export default {
  ...
  methods: {
    ...
    initMailboxTemplate (formInline) {
      backstageMailboxTemplateHttp(formInline).then(res => {
    ...
    storeOrUpdate () {
      this.$confirm('Are you sure?', 'Prompt', {
        ...
        backstageMailboxTemplateStoreHttp(this.mailboxTemplateForm).then(res => {
          element.message({
            type: 'success',
            message: 'success'
          })
